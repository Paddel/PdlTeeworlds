...
static int gs_TextureInvBlob = -1;
	...
	)
{
	...
	if(m_pClient->m_pControls->m_BlockingPlayer == pInfo.m_ClientID)
	{
		...
		IGraphics::CQuadItem QuadItem(Position.x, Position.y, 64+32,64+32);
		Graphics()->QuadsDraw(&QuadItem, 1);

		Graphics()->QuadsEnd();
	}

	if (g_Config.m_PdlGrenadeCooldown)
	{
		float a = (Client()->GameTick() - Player.m_AttackTick + Client()->PredIntraGameTick()) / 50.0f;
		float Rate = (0.5f - a) * 2.0f;
		if (Rate > 0.0f)
		{
			Graphics()->TextureSet(-1);
			Graphics()->LinesBegin();

			float CircleRate = 2*pi - Rate * 2 * pi;
			for (int j = 0; j < 4; j++)
	...
	{
		RenderInfo.m_ColorFeet.a = RenderInfo.m_GotAirJump ? 1.0f : 0.5f;
		RenderInfo.m_ColorBody.a = RenderInfo.m_GotAirJump ? 1.0f : 0.5f;
	}
...
{
	gs_TextureInvBlob = Graphics()->LoadTexture("pdl_invblob.png", IStorage::TYPE_ALL, CImageInfo::FORMAT_AUTO, 0);
}
