...
void CMenus::RenderPlayers(CUIRect MainView)
{
	...
	int OldSelected = -1;
	UiDoListboxStart(&s_ScrollValue, &Options, 20.0f, Localize(""), "", NumPlayers, 1, OldSelected, s_ScrollValue);
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
		...
		Item.m_Rect.HSplitTop(20.0f, &ButtonBar, &Options);
		if(Count%2 == 0)
		...
		// player info
		Player.VSplitLeft(20.0f, &Button, &Player);
		...
		if(m_pClient->m_aClients[Index].m_Friend == 1)
			TextRender()->TextColor(0.2f, 1.0f, 0.2f, 1.0f);

		CTextCursor Cursor;
		TextRender()->SetCursor(&Cursor, Player.x, Player.y, 12.0f, TEXTFLAG_RENDER|TEXTFLAG_STOP_AT_END);
