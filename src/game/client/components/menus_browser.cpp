...
void CMenus::RenderServerbrowserServerList(CUIRect View)
{
	...
	for (int i = 0; i < NumServers; i++)
	{
		...
		if(Row.y+Row.h > OriginalView.y && Row.y < OriginalView.y+OriginalView.h)
		{
			if(Selected)
			{
				...
				vec3 BarColor = vec3(45/255.0f, 254/255.0f, 184/255.0f);//

				IGraphics::CColorVertex Color[4] = {
					IGraphics::CColorVertex(0, BarColor.r, BarColor.g, BarColor.b, 0.5f),
					IGraphics::CColorVertex(1, BarColor.r, BarColor.g, BarColor.b, 0.15f),
					IGraphics::CColorVertex(2, BarColor.r, BarColor.g, BarColor.b, 0.15f),
					IGraphics::CColorVertex(3, BarColor.r, BarColor.g, BarColor.b, 0.5f)};
				Graphics()->SetColorVertex(Color, 4);
				IGraphics::CQuadItem QuadFadeLeft(NewRect.x, NewRect.y, NewRect.w, NewRect.h);
				Graphics()->QuadsDrawTL(&QuadFadeLeft, 1);
				...
				Graphics()->SetColor(BarColor.r, BarColor.g, BarColor.b, 0.5f);//45/255.0f, 254/255.0f, 184/255.0f

				IGraphics::CTriangleItem TriangleTop(NewRect.x, NewRect.y, NewRect.x, NewRect.y + NewRect.h/2, NewRect.x - n, NewRect.y);
				Graphics()->TriangleDraw(&TriangleTop, 1);

				IGraphics::CTriangleItem TriangleDown(NewRect.x, NewRect.y + NewRect.h/2, NewRect.x, NewRect.y + NewRect.h, NewRect.x - n, NewRect.y + NewRect.h);
				Graphics()->TriangleDraw(&TriangleDown, 1);
	...
	// clear button
	{
		...
		RenderTools()->DrawUIRect(&Button, vec4(1,1,1,0.53f)*ButtonColorMul(&s_ClearButton), CUI::CORNER_R, 3.0f);
	...
	if(Client()->GetAutojoin())
	{
		char aAutojoinTxt[256];
		char aAutojoinAddr[256];
		net_addr_str(Client()->GetAutojoinAddr(), aAutojoinAddr, sizeof(aAutojoinAddr), true);
		str_format(aAutojoinTxt, sizeof(aAutojoinTxt), "Autojoin: %s", aAutojoinAddr);
		Button.VSplitLeft(32.0f, 0, &Button);
		TextRender()->TextColor(0.0f, 0.7f, 0.3f, 1.0f);
		UI()->DoLabelScaled(&Button, aAutojoinTxt, 12.0f, -1);
