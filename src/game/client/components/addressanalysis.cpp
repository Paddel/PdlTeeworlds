...
CAddressAnalysis::CAddressAnalysis()
{
	mem_zero(&m_aaAddresRanges, sizeof(m_aaAddresRanges));
	m_NumRanges = 0;
}
...
void CAddressAnalysis::ResultWhoisRangeAddr(int Index, char *pResult, int pResultSize, void *pData, int Row, int MaxRows)
{
	...
	for (int i = 0; i < pThis->m_NumRanges; i++)
		if (str_comp(pThis->m_aaAddresRanges[i], pResult) == 0)
			return;

	str_copy(pThis->m_aaAddresRanges[pThis->m_NumRanges++], pResult, sizeof(pThis->m_aaAddresRanges[0]));
