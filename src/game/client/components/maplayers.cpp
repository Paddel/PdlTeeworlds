...
static int gs_TextureGamelayerVanilla = -1;
static int gs_TextureGamelayerDDRace = -1;
...
void CMapLayers::InitTextures()
{
	gs_TextureGamelayerVanilla = Graphics()->LoadTexture("editor/entities.png", IStorage::TYPE_ALL, CImageInfo::FORMAT_AUTO, 0);
	gs_TextureGamelayerDDRace = Graphics()->LoadTexture("editor/entities_ddrace.png", IStorage::TYPE_ALL, CImageInfo::FORMAT_AUTO, 0);
}
...
void CMapLayers::Render(vec2 Center, float Zoom, bool UseClip)
{
	...
	for(int g = 0; g < m_pLayers->NumGroups(); g++)
	{
		...
		if(!g_Config.m_GfxNoclip && pGroup->m_Version >= 2 && pGroup->m_UseClipping)
		{
			// set clipping
			float Points[4];
			MapScreenToGroup(Center.x, Center.y, m_pLayers->GameGroup(), Zoom);
		...
		MapScreenToGroup(Center.x, Center.y, pGroup, Zoom);
		...
		for(int l = 0; l < pGroup->m_NumLayers; l++)
		{
			...
			if(g_Config.m_PdlGamelayer && g_Config.m_PdlGamelayerBack)
				Render = false;
		...
		if(!g_Config.m_GfxNoclip && UseClip == false)
...
void CMapLayers::Render(vec2 Center, CUIRect Clip, float Zoom)
{
	float Points[4];
	MapScreenToGroup(Center.x, Center.y, m_pLayers->GameGroup(), 1.0f);
	Graphics()->GetScreen(&Points[0], &Points[1], &Points[2], &Points[3]);
	
	float ScreenWidth = Points[2]-Points[0];
	float ScreenHeight = Points[3]-Points[1];

	float XScale = ScreenWidth/Graphics()->ScreenWidth();
	float YScale = ScreenHeight/Graphics()->ScreenHeight();

	Center += vec2(ScreenWidth*0.5f-(Clip.w*0.5f+Clip.x)*XScale, ScreenHeight*0.5f-(Clip.h*0.5f+Clip.y)*YScale)*Zoom;

	Render(Center, Zoom, true);
}
...
void CMapLayers::RenderGamelayer(vec2 Center)
{
	if(m_Type == 0)
	{
		if(g_Config.m_PdlGamelayerBack)
		{
			float Points[4];
			RenderTools()->MapscreenToWorld(Center.x, Center.y, 1.0f, 1.0f,
				0, 0, Graphics()->ScreenAspect(), m_pClient->m_pCamera->m_Zoom, Points);
			Graphics()->MapScreen(Points[0], Points[1], Points[2], Points[3]);

			float ScreenWidth = Points[2]-Center.x;
			float ScreenHeight = Points[3]-Center.y;
			Graphics()->TextureSet(-1);

			Graphics()->QuadsBegin();
			Graphics()->SetColor(0, 0, 0, 1);
			IGraphics::CQuadItem QuadItem(Center.x-ScreenWidth, Center.y-ScreenHeight, ScreenWidth*2, ScreenHeight*2);
	...
	{
		CMapItemGroup *pGroup = m_pLayers->GameGroup();

		MapScreenToGroup(Center.x, Center.y, pGroup, 1.0f);

		CMapItemLayer *pLayer = (CMapItemLayer *)m_pLayers->GameLayer();

		CMapItemLayerTilemap *pTMap = (CMapItemLayerTilemap *)pLayer;
		if(Client()->IsDDRace())
			Graphics()->TextureSet(gs_TextureGamelayerDDRace);
		else
			Graphics()->TextureSet(gs_TextureGamelayerVanilla);
