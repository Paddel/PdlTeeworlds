...
{
	bool Alt = m_pBinds->GetInput()->KeyPressed(KEY_LALT);
	// don't handle invalid events and keys that arn't set to anything
	if(Event.m_Key >= KEY_F1 && Event.m_Key <= KEY_F15 && m_pBinds->m_aaKeyBindings[Event.m_Key](Alt)[0] != 0)
	{
		int Stroke = 0;
		if(Event.m_Flags&IInput::FLAG_PRESS)
			Stroke = 1;

		m_pBinds->GetConsole()->ExecuteLineStroked(Stroke, m_pBinds->m_aaKeyBindings[Event.m_Key](Alt));
		return true;
	}
...
{
	if(KeyID < 0 || KeyID >= KEY_LAST || KeyID == KEY_LALT)
		return;
...
{
	bool Alt = Input()->KeyPressed(KEY_LALT);
	// don't handle invalid events and keys that arn't set to anything
	if(e.m_Key <= 0 || e.m_Key >= KEY_LAST || m_aaKeyBindings[e.m_Key](Alt)[0] == 0)
		return false;
	
	int Stroke = 0;
	if(e.m_Flags&IInput::FLAG_PRESS)
		Stroke = 1;
	Console()->ExecuteLineStroked(Stroke, m_aaKeyBindings[e.m_Key](Alt));
	return true;
}
...
void CBinds::UnbindAll()
{
	...
	{
		m_aaKeyBindings[i].m_aCode[0] = 0;
		m_aaKeyBindings[i].m_aAltCode[0] = 0;
	}
}
...
void CBinds::SetDefaults()
{
	...
	Bind(KEY_MOUSE_1, "+fire", true);
	Bind(KEY_MOUSE_2, "+hook", true);
...
void CBinds::OnConsoleInit()
{
	...
	Console()->Register("bind_alt", "sr", CFGFLAG_CLIENT, ConBindAlt, this, "Bind alternativ key to execute the command");
	Console()->Register("bind", "sr", CFGFLAG_CLIENT, ConBind, this, "Bind key to execute the command");
	Console()->Register("unbind", "s?s", CFGFLAG_CLIENT, ConUnbind, this, "Unbind key");
...
void CBinds::ConBindAlt(IConsole::IResult *pResult, void *pUserData)
{
	bool Alt = false;
	CBinds *pBinds = (CBinds *)pUserData;
	const char *pKeyName = pResult->GetString(0);
...
void CBinds::ConUnbind(IConsole::IResult *pResult, void *pUserData)
{
	...
	CBinds *pBinds = (CBinds *)pUserData;
	const char *pKeyName = pResult->GetString(0);
	if (str_comp(pKeyName, "alt") == 0)
	{
		pKeyName = pResult->GetString(1);
		Alt = true;
	}
...
void CBinds::ConfigSaveCallback(IConfig *pConfig, void *pUserData)
{
	...
	for(int i = 0; i < KEY_LAST; i++)
	{
		...
		{
			bool Alt = j == 1;
			if (pSelf->m_aaKeyBindings[i](Alt)[0])
			{
				str_format(aBuffer, sizeof(aBuffer), "bind%s %s ", Alt? "_alt" : "", pSelf->Input()->KeyName(i));

				// process the string. we need to escape some characters
				const char *pSrc = pSelf->m_aaKeyBindings[i](Alt);
