...
void CMenus::RenderSettingsTee(CUIRect MainView)
{
	...
	{
		str_copy(g_Config.m_PlayerSkin, "limekitty", sizeof(g_Config.m_PlayerSkin));
...
void CMenus::RenderSettingsGraphics(CUIRect MainView)
{
	...
	if(DoButton_Menu(&s_ApplyButton, Localize("Apply"), m_NeedRestartGraphics?0:-1, &Button))
	{
		...
		g_Config.m_GfxScreenWidth = s_GfxScreenWidth;
		g_Config.m_GfxScreenHeight = s_GfxScreenHeight;
		g_Config.m_GfxColorDepth = s_GfxColorDepth;
		g_Config.m_GfxBorderless = s_GfxBorderless;
		g_Config.m_GfxFullscreen = s_GfxFullscreen;
		g_Config.m_GfxVsync = s_GfxVsync;
...
void CMenus::RenderSettingsSound(CUIRect MainView)
{
	...
	if(DoButton_CheckBox(&g_Config.m_SndEnable, Localize("Use sounds"), g_Config.m_SndEnable, &Button))
	{
		...
		if(g_Config.m_SndEnable)
		{
			...
			m_pClient->m_pSounds->Stop(SOUND_MENU_ALT);
	...
	MainView.HSplitTop(20.0f, &Button, &MainView);
	static int s_ApplyButton = -1;
	if(DoButton_Menu(&s_ApplyButton, Localize("Apply"), m_NeedRestartSound?0:-1, &Button))
	{
		s_SndEnable = g_Config.m_SndRate;
		s_SndRate = g_Config.m_SndRate;

		Client()->ReinitWindow();

		g_Config.m_SndRate = s_SndEnable;
		g_Config.m_SndRate = s_SndRate;
