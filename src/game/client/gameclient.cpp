...
#include "components/blockscore.h"
...
static CBlockScore gs_BlockScore;
...
void CGameClient::OnConsoleInit()
{
	...
	m_All.Add(m_pBlockScore);
	m_All.Add(m_pAutoRun);
	m_All.Add(&m_pParticles->m_RenderExplosions);
	m_All.Add(&gs_NamePlates);
	m_All.Add(m_pBlockHelp);
	...
	m_Input.Add(m_pBlockScore);
	m_Input.Add(m_pAutoRun);
	...
	Console()->Register("pdl_dummyinfo_skin", "is", CFGFLAG_CLIENT, ConDummyinfoSkin, this, "Skin of Dummy");
...
void CGameClient::InitTextures()
{
	// load default font
	static CFont *pDefaultFont = 0;
	static CFont *pBubbleFont = 0;
	char aFilename[512];
	...
	{
		io_close(File);
		pBubbleFont = TextRender()->LoadFont(aFilename);
...
static int64 s_SentTime = 0;
static float s_ClockSkinNum = 30.0f;

void CGameClient::DoClockStart()
{
	...
	for(int i = 0; i < 3; i++)
	{
		...
		ColorBody <<= 8;
		ColorBody += clamp((int)(k*255), 0, 255);
	}
	...
	for(int i = 0; i < 3; i++)
	{
		...
		ColorFeet <<= 8;
		ColorFeet += clamp((int)(k*255), 0, 255);
	}
...
void CGameClient::OnNewSnapshot()
{
	...
	// go trough all the items in the snapshot and gather the info we want
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			{
				CNetObj_PlayerInfo *pInfo = (CNetObj_PlayerInfo *)pData;
...
void CGameClient::PredictSnap(CSnapState *pSnap)
{
	// store the previous values so we can detect prediction errors
	CCharacterCore BeforePrevChar = m_PredictedPrevChar;
	CCharacterCore BeforeChar = m_PredictedChar;

	if(pSnap->m_LocalClientID < 0 || pSnap->m_LocalClientID >= MAX_CLIENTS)
		return;

	// we can't predict without our own id or own character
	if(!pSnap->m_aCharacters[pSnap->m_LocalClientID].m_Active)
		return;

	// don't predict anything if we are paused
	if(pSnap->m_pGameInfoObj && pSnap->m_pGameInfoObj->m_GameStateFlags&GAMESTATEFLAG_PAUSED)
	{
		if(pSnap->m_pLocalCharacter)
			m_PredictedChar.Read(pSnap->m_pLocalCharacter);
		if(pSnap->m_pLocalPrevCharacter)
			m_PredictedPrevChar.Read(pSnap->m_pLocalPrevCharacter);
		return;
	}
	...
	{
		// fetch the local
		if(Tick == Client()->PredGameTick() && World.m_apCharacters[pSnap->m_LocalClientID])
			m_PredictedPrevChar = *World.m_apCharacters[pSnap->m_LocalClientID];
		...
		if(Tick > m_LastNewPredictedTick)
		{
			...
			if(pSnap->m_LocalClientID != -1 && World.m_apCharacters[pSnap->m_LocalClientID])
			{
				vec2 Pos = World.m_apCharacters[pSnap->m_LocalClientID]->m_Pos;
				int Events = World.m_apCharacters[pSnap->m_LocalClientID]->m_TriggeredEvents;
		...
		if(Tick == Client()->PredGameTick() && World.m_apCharacters[pSnap->m_LocalClientID])
			m_PredictedChar = *World.m_apCharacters[pSnap->m_LocalClientID];
	}
...
void CGameClient::CClientData::Reset()
{
	...
	mem_zero(&m_BlockScore, sizeof(m_BlockScore));
	m_BlockLastInteraction = -1;
	m_Blocked = 0;
	m_BlockType = 0;
	m_aAddr[0] = 0;
	UpdateRenderInfo();
...
void CGameClient::RenderSecondaryGame(CUIRect Rect, vec2 Center, vec2 CenterShift, float Zoom)
{
	...
	//UI()->ClipEnable(&Rect);
	Graphics()->ClipEnable(Rect.x, Rect.y, Rect.w, Rect.h);
	...
	//vec2 CameraCenter;// = Center+CenterShift*Zoom;//-vec2(Rect.x, Rect.y);

	if(m_pMapLayersBackGround)
		m_pMapLayersBackGround->Render(Center, Rect, Zoom);

	m_pParticles->m_RenderTrail.OnRender();
	gs_Players.OnRender();

	if(m_pMapLayersForeGround)
		m_pMapLayersForeGround->Render(Center, Rect, Zoom);

	m_pItems->OnRender();
	m_pParticles->m_RenderExplosions.OnRender();
	gs_NamePlates.OnRender();
	m_pParticles->m_RenderGeneral.OnRender();
...
void CGameClient::ConDummyinfoName(IConsole::IResult *pResult, void *pUserData)
{
	...
	str_copy(pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_aName, pResult->GetString(1), sizeof(pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_aName));
}

void CGameClient::ConDummyinfoClan(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *pGameClient = (CGameClient*) pUserData;
	int DummyID = pResult->GetInteger(0);
	if(DummyID < 0 || DummyID >= MAX_DUMMIES)
	...
	str_copy(pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_aClan, pResult->GetString(1), sizeof(pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_aClan));
}

void CGameClient::ConDummyinfoCountry(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *pGameClient = (CGameClient*) pUserData;
	int DummyID = pResult->GetInteger(0);
	if(DummyID < 0 || DummyID >= MAX_DUMMIES)
	...
	pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_Country = pResult->GetInteger(1);
}

void CGameClient::ConDummyinfoSkin(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *pGameClient = (CGameClient*) pUserData;
	int DummyID = pResult->GetInteger(0);
	if(DummyID < 0 || DummyID >= MAX_DUMMIES)
	...
	str_copy(pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_aSkin, pResult->GetString(1), sizeof(pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_aSkin));
}

void CGameClient::ConDummyinfoCostumColor(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *pGameClient = (CGameClient*) pUserData;
	int DummyID = pResult->GetInteger(0);
	if(DummyID < 0 || DummyID >= MAX_DUMMIES)
...
void CGameClient::ConDummyinfoColorBody(IConsole::IResult *pResult, void *pUserData)
{
	...
	pGameClient->m_aDummyData[DummyID].m_PlayerInfo.m_ColorBody = pResult->GetInteger(1);
}

void CGameClient::ConDummyinfoColorFeet(IConsole::IResult *pResult, void *pUserData)
{
	CGameClient *pGameClient = (CGameClient*) pUserData;
	int DummyID = pResult->GetInteger(0);
	if(DummyID < 0 || DummyID >= MAX_DUMMIES)
