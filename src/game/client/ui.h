...
class CUI
{
	...
	CGameClient *m_pClient;//used for sound
...
public:
	...
	int MouseInside(const CUIRect *pRect) { return MouseInside(pRect, vec2(m_MouseX, m_MouseY)); }
	int MouseInside(const CUIRect *pRect, vec2 Pos);
	...
	void SetClient(CGameClient *pClient) { m_pClient = pClient; }
};
