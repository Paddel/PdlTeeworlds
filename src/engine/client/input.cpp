...
float CInput::JoystickAxes(int Axe)
{
	if(Axe < 0 || Axe >= SDL_JoystickNumAxes(m_pJoystick))
		return 0.0f;

	return SDL_JoystickGetAxis(m_pJoystick, Axe)/(Axe == 2? 32639.0f : 32767.0f);
}

int CInput::JoystickButton(int Button)
{
...
int CInput::Update()
{
	static bool s_LastWindowActive = Graphics()->WindowActive();
	...
	//Reset the state -- unbug sdl
	if (s_LastWindowActive == false && Graphics()->WindowActive())
	{
		int Size;
		Uint8 *pState = SDL_GetKeyState(&Size);
		mem_zero(pState, Size);
	}

	s_LastWindowActive = Graphics()->WindowActive();
