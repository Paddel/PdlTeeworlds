...
class CTextRender : public IEngineTextRender
{
	...
	void InitTexture(CFontSizeData *pSizeData, int CharWidth, int CharHeight, int Xchars, int Ychars)
	{
		...
		for(int i = 0; i < 2; i++)
		{
			...
			//Graphics()->PdlLoadTexture(&pSizeData->m_aTextures[i], Width, Height, CImageInfo::FORMAT_ALPHA, pMem, CImageInfo::FORMAT_ALPHA, IGraphics::TEXLOAD_NOMIPMAPS);
...
public:
	...
	virtual void SetBubbleFont(CFont *pFont)
	{
		dbg_msg("textrender", "bubble pFont set %p", pFont);
		m_pBubbleFont = pFont;
	}
	...
	virtual void GetTextColor(float *pR, float *pG, float *pB, float *pA)
	{
		*pR = m_TextR;
		*pG = m_TextG;
		*pB = m_TextB;
