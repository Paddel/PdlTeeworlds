...
void CGraphics_OpenGL::Flush()
{
	...
	if(m_RenderEnable)
	{
		if(m_Drawing == DRAWING_LINES)
			glDrawArrays(GL_LINES, 0, m_NumVertices);
		else if(m_Drawing == DRAWING_TRIANGLE)
			glDrawArrays(GL_TRIANGLES, 0, m_NumVertices);
		else if(m_Drawing == DRAWING_QUADS)
...
void CGraphics_OpenGL::TriangleBegin()
{
	dbg_assert(m_Drawing == 0, "called Graphics()->TriangleBegin twice");
	m_Drawing = DRAWING_TRIANGLE;
	TriangleSetRotation(0);
	SetColor(1,1,1,1);
}

void CGraphics_OpenGL::TriangleEnd()
{
	dbg_assert(m_Drawing == DRAWING_TRIANGLE, "called Graphics()->TriangleEnd without begin");
	Flush();
	m_Drawing = 0;
}

void CGraphics_OpenGL::TriangleSetRotation(float Angle)
{
	dbg_assert(m_Drawing == DRAWING_TRIANGLE, "called Graphics()->TriangleSetRotation without begin");
	m_Rotation = Angle;
}

void CGraphics_OpenGL::TriangleDraw(const CTriangleItem *pArray, int Num)
{
	dbg_assert(m_Drawing == DRAWING_TRIANGLE, "called Graphics()->TriangleDraw without begin");
	...
	for(int i = 0; i < Num; ++i)
	{
		...
		m_aVertices[m_NumVertices + 2*i + 2].m_Pos.x = pArray[i].m_X2;
		m_aVertices[m_NumVertices + 2*i + 2].m_Pos.y = pArray[i].m_Y2;
		m_aVertices[m_NumVertices + 2*i + 2].m_Tex = m_aTexture[2];
		m_aVertices[m_NumVertices + 2*i + 2].m_Color = m_aColor[2];

		if(m_Rotation != 0)
		{
			Center.x = pArray[i].m_X0;
			Center.y = pArray[i].m_Y0;

			Rotate4(Center, &m_aVertices[m_NumVertices + 3*i]);
		}
	}
