...
static void Mix(short *pFinalOut, unsigned Frames)
{
	...
	for(unsigned i = 0; i < NUM_VOICES; i++)
	{
		if(m_aVoices[i].m_pSample)
		{
			...
			if(v->m_Flags&ISound::FLAG_RESPLEN)
			{
				//dbg_msg(0, "%f %f", (float)v->m_Tick/v->m_pSample->m_NumFrames, (float)time_get()/time_freq());
			}
...
int CSound::Init()
{
	...
	m_pGraphics->AddTextureUser(this);
