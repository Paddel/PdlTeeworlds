...
CClient::CClient() : m_DemoPlayer(&m_SnapshotDelta), m_DemoRecorder(&m_SnapshotDelta)
{
	...
	{
		m_aDummy[i].m_Online = false;
		m_aDummy[i].m_Inited = false;
		m_aDummy[i].m_LastConnect = 0;
		m_aDummy[i].m_SnapshotStorage.Init();
	}
...
//dummy
void CClient::DummyToggle(int Index)
{
	if(Index < 0 || Index >= MAX_DUMMIES)
		return;

	if(m_aDummy[Index].m_Online)
		DummyDisonnect(Index);
	else
		DummyConnect(Index);
}

void CClient::DummyConnect(int Index)
{
	if(Index < 0 || Index >= MAX_DUMMIES || m_aDummy[Index].m_Online)
		return;

	m_aDummy[Index].m_SnapshotParts = 0;
	m_aDummy[Index].m_RecivedSnapshots = 0;
	m_aDummy[Index].m_SnapshotStorage.PurgeAll();
	m_aDummy[Index].m_CurrentRecvTick;
	m_aDummy[Index].m_ClientID = -1;

	if(m_aDummy[Index].m_Inited == false)
	{

		m_aDummy[Index].m_Inited = true;

		NETADDR BindAddr;
		mem_zero(&BindAddr, sizeof(BindAddr));
		BindAddr.type = NETTYPE_ALL;
		if(!m_aDummy[Index].m_NetClient.Open(BindAddr, 0))
		{
...
void CClient::DummyDisonnect(int Index)
{
	...
	m_aDummy[Index].m_Online = false;
	m_aDummy[Index].m_NetClient.Disconnect(0);

	if(m_DummyCamera == Index)
	{
		m_DummyCamera = -1;
		//next dummy
		for(int i = 0; i < MAX_DUMMIES; i++)
...
{
	IOHANDLE DummyFile = Storage()->OpenFile("pdl_dummies.cfg", IOFLAG_WRITE, IStorage::TYPE_SAVE);
	...
	for(int i = 0; i < MAX_DUMMIES; i++)
	{
		char aFillingStr[128];
		mem_zero(&aFillingStr, sizeof(aFillingStr));
		IGameClient::CPlayerInfo *pPlayerInfo = GameClient()->GetDummyPlayerInfo(i);

		str_format(aFillingStr, sizeof(aFillingStr), "pdl_dummyinfo_name %i \"%s\"", i, pPlayerInfo->m_aName);
		io_write(DummyFile, aFillingStr, str_length(aFillingStr));
		io_write_newline(DummyFile);

		str_format(aFillingStr, sizeof(aFillingStr), "pdl_dummyinfo_clan %i \"%s\"", i, pPlayerInfo->m_aClan);
		io_write(DummyFile, aFillingStr, str_length(aFillingStr));
		io_write_newline(DummyFile);

		str_format(aFillingStr, sizeof(aFillingStr), "pdl_dummyinfo_country %i %i", i, pPlayerInfo->m_Country);
		io_write(DummyFile, aFillingStr, str_length(aFillingStr));
		io_write_newline(DummyFile);

		str_format(aFillingStr, sizeof(aFillingStr), "pdl_dummyinfo_skin %i \"%s\"", i, pPlayerInfo->m_aSkin);
		io_write(DummyFile, aFillingStr, str_length(aFillingStr));
		io_write_newline(DummyFile);
		...
		str_format(aFillingStr, sizeof(aFillingStr), "pdl_dummyinfo_color_body %i %i", i, pPlayerInfo->m_ColorBody);
		io_write(DummyFile, aFillingStr, str_length(aFillingStr));
		io_write_newline(DummyFile);

		str_format(aFillingStr, sizeof(aFillingStr), "pdl_dummyinfo_color_feet %i %i", i, pPlayerInfo->m_ColorFeet);
		io_write(DummyFile, aFillingStr, str_length(aFillingStr));
		io_write_newline(DummyFile);
	}
...
// ----- send functions -----
int CClient::SendMsg(CMsgPacker *pMsg, int Flags, bool System)
{
	return SendMsgEx(pMsg, Flags, System);
}
...
int CClient::SendMsgDummy(CMsgPacker *pMsg, int Flags, int Index, bool System)
{
	if(Index < 0 || Index >= MAX_DUMMIES)
		return 0;
	...
	if(State() == IClient::STATE_OFFLINE || m_aDummy[Index].m_Online == false)
		return 0;
	...
	*((unsigned char*)Packet.m_pData) <<= 1;
	...
	if(!(Flags&MSGFLAG_NOSEND))
		m_aDummy[Index].m_NetClient.Send(&Packet);
	return 0;
}
...
void CClient::SendInput()
{
	...
	{
		m_aInputList[i] = m_aInputs[m_CurrentInput].m_aData[i];
		Msg.AddInt(m_aInputList[i]);
	}

	SendMsgSpread(&Msg, MSGFLAG_FLUSH, true);
...
void CClient::OnEnterGame()
{
	...
	m_SnapshotStorageMixed.PurgeAll();
	m_RecivedSnapshots = 0;
	m_SnapshotParts = 0;
	m_PredTick = 0;
	m_CurrentRecvTick = 0;
...
void CClient::Connect(const char *pAddress)
{
	...
	m_MapdlownloadNextType = g_Config.m_PdlQuickDownload;
...
void CClient::DisconnectWithReason(const char *pReason)
{
	...
	if(m_MapdownloadQuick && str_find(m_NetClient.ErrorString(), "out of buffer"))
		Redownload = true;
	...
	m_MapdownloadQuick = false;

	if(Redownload)
	{
		Connect(m_aServerAddressStr);
		m_MapdlownloadNextType = 0;
	}

	if(g_Config.m_PdlAutojoin)
	{
		if(str_find_nocase(m_NetClient.ErrorString(), "server is full"))
		{
			m_Autojoin = true;
			m_AutojoinAddress = m_ServerAddress;
		}
	}
...
void *CClient::SnapFindItem(int SnapID, int Type, int ID)
{
	...
	/*for (int j = 0; j < MAX_DUMMIES; j++)
	{
		if (m_aDummy[j].m_Online && m_aDummy[j].m_RecivedSnapshots >= 3)
		{
			CSnapshotStorage::CHolder *pDummySnapshot;
			if(SnapID == SNAP_CURRENT)
				pDummySnapshot = m_aDummy[j].m_SnapshotStorage.m_pLast;
			else if(SnapID == SNAP_PREV)
				pDummySnapshot = m_aDummy[j].m_SnapshotStorage.m_pFirst;

			for (int i = 0; i < pDummySnapshot->m_pSnap->NumItems(); i++)
			{
				CSnapshotItem *pItem = pDummySnapshot->m_pAltSnap->GetItem(i);
				if (pItem->Type() == Type && pItem->ID() == ID)
					return (void *)pItem->Data();
			}
		}
...
void CClient::SnapSetStaticsize(int ItemType, int Size)
{
	...
	for(int i = 0; i < MAX_DUMMIES; i++)
		m_aDummy[i].m_SnapshotDelta.SetStaticsize(ItemType, Size);
}
...
void CClient::ProcessConnlessPacket(CNetChunk *pPacket)
{
	...
	if (pPacket->m_DataSize >= (int)sizeof(SERVERBROWSE_INFO) && mem_comp(pPacket->m_pData, SERVERBROWSE_INFO, sizeof(SERVERBROWSE_INFO)) == 0)
	{
		...
		if (!Up.Error())
		{
			...
			if(m_Autojoin && net_addr_comp(&pPacket->m_Address, &m_AutojoinAddress) == 0)
			{
				if(Info.m_NumClients < Info.m_MaxClients)
				{
					char aAddr[256];
					net_addr_str(&m_AutojoinAddress, aAddr, sizeof(aAddr), true);
					Connect(aAddr);
				}
				return;
			}
...
void CClient::ProcessServerPacketDummy(CNetChunk *pPacket, int Index)
{
	CUnpacker Unpacker;
	...
	if (Sys)
	{
		if (Msg == NETMSG_MAP_CHANGE)
		{
			...
			SendMsgDummy(&MsgReady, MSGFLAG_VITAL | MSGFLAG_FLUSH, Index);
		}
		else if (Msg == NETMSG_MAP_DATA)
		{
			...
			Msg.AddInt(Chunk + 1);
			SendMsgDummy(&Msg, MSGFLAG_VITAL | MSGFLAG_FLUSH, Index);*/
		}
		else if (Msg == NETMSG_CON_READY)
		{
			...
			SendMsgDummy(&MsgRcon, MSGFLAG_VITAL, Index);

			CNetMsg_Cl_StartInfo MsgInfo;
			MsgInfo.m_pName = pPlayerInfo->m_aName;
		...
		else if (Msg == NETMSG_SNAP || Msg == NETMSG_SNAPSINGLE || Msg == NETMSG_SNAPEMPTY)
		{
			...
			{
				if (m_aDummy[Index].m_RecivedSnapshots == 0)
				{
					CMsgPacker Msg(NETMSG_INPUT);
					Msg.AddInt(GameTick);
					SendMsgDummy(&Msg, MSGFLAG_VITAL | MSGFLAG_FLUSH, Index);
			...
			if (GameTick >= m_aDummy[Index].m_CurrentRecvTick)
			{
				if (GameTick != m_aDummy[Index].m_CurrentRecvTick)
				{
					m_aDummy[Index].m_SnapshotParts = 0;
					m_aDummy[Index].m_CurrentRecvTick = GameTick;
				}

				mem_copy((char*)m_aDummy[Index].m_aSnapshotIncommingData + Part*MAX_SNAPSHOT_PACKSIZE, pData, PartSize);
				m_aDummy[Index].m_SnapshotParts |= 1 << Part;

				if (m_aDummy[Index].m_SnapshotParts == (unsigned)((1 << NumParts) - 1))
				{
					...
					unsigned char aTmpBuffer4[CSnapshot::MAX_SIZE];
					CSnapshot *pTmpBuffer3 = (CSnapshot*)aTmpBuffer3;	// Fix compiler warning for strict-aliasing
					CSnapshot *pTmpBuffer4 = (CSnapshot*)aTmpBuffer3;
					int SnapSize;
					int SnapSizeMixed;
					...
					if (DeltaTick >= 0)
					{
						int DeltashotSize = m_aDummy[Index].m_SnapshotStorage.Get(DeltaTick, 0, &pDeltaShot, 0);
						...
						if (DeltashotSize < 0)
						{
							...
							if (g_Config.m_Debug)
							{
								...
								m_pConsole->Print(IConsole::OUTPUT_LEVEL_DEBUG, IConsole::OUTPUTTYPE_ERROR, "dummy", aBuf);
							}
					...
					// decompress snapshot
					pDeltaData = m_aDummy[Index].m_SnapshotDelta.EmptyDelta();
					DeltaSize = sizeof(int) * 3;

					if (CompleteSize)
					{
						int IntSize = CVariableInt::Decompress(m_aDummy[Index].m_aSnapshotIncommingData, CompleteSize, aTmpBuffer2);
					...
					// unpack delta
					SnapSize = m_aDummy[Index].m_SnapshotDelta.UnpackDelta(pDeltaShot, pTmpBuffer3, pDeltaData, DeltaSize);
					if (SnapSize < 0)
					{
						m_pConsole->Print(IConsole::OUTPUT_LEVEL_DEBUG, IConsole::OUTPUTTYPE_ERROR, "dummy", "delta unpack failed!");
						return;
					}
					...
					if (Msg != NETMSG_SNAPEMPTY && pTmpBuffer3->Crc() != Crc)
					{
						if (g_Config.m_Debug)
						{
							...
							str_format(aBuf, sizeof(aBuf), "snapshot crc error #%d - tick=%d wantedcrc=%d gotcrc=%d compressed_size=%d delta_tick=%d",
								m_aDummy[Index].m_SnapCrcErrors, GameTick, Crc, pTmpBuffer3->Crc(), CompleteSize, DeltaTick);
							m_pConsole->Print(IConsole::OUTPUT_LEVEL_DEBUG, IConsole::OUTPUTTYPE_ERROR, "dummy", aBuf);
						}

						m_aDummy[Index].m_SnapCrcErrors++;
						if (m_aDummy[Index].m_SnapCrcErrors > 10)
						{
							...
							m_aDummy[Index].m_CurrentRecvTick = -1;
							m_aDummy[Index].m_SnapCrcErrors = 0;
						}
						return;
					}
					else
					{
						if (m_aDummy[Index].m_SnapCrcErrors)
							m_aDummy[Index].m_SnapCrcErrors--;
					}
					...
					m_aDummy[Index].m_SnapshotStorage.PurgeUntil(PurgeTick);
					//m_SnapshotStorageMixed.PurgeUntil(PurgeTick);

					SnapSizeMixed = SnapSize;
					pTmpBuffer4->Copy(pTmpBuffer3);

					//if (m_aDummy[0].m_Online && m_aDummy[0].m_RecivedSnapshots >= 3)
					{
						//if (m_aDummy[0].m_CurrentRecvTick > m_CurrentRecvTick)
						{
							/*CSnapshotStorage::CHolder *pDummySnapshot = m_SnapshotStorage.m_pLast;
							SnapSizeMixed = pTmpBuffer4->Add(pDummySnapshot->m_pSnap);*/
						}
					}

					// add new
					m_aDummy[Index].m_SnapshotStorage.Add(GameTick, time_get(), SnapSize, pTmpBuffer3, 1);

					//update playerinfo
					m_aDummy[Index].m_ClientID = -1;
					for (int i = 0; i < pTmpBuffer3->NumItems(); i++)
					{
						CSnapshotItem *pItem = pTmpBuffer4->GetItem(i);
						if (pItem->Type() != NETOBJTYPE_PLAYERINFO)
							continue;
						
						CNetObj_PlayerInfo *pPlayerInfo = (CNetObj_PlayerInfo *)pItem->Data();
						if (pPlayerInfo->m_Local == 0)
							continue;

						m_aDummy[Index].m_ClientID = pItem->ID();
						//pPlayerInfo->m_Local = 0;
						break;
					}

					//m_SnapshotStorageMixed.Add(GameTick, time_get(), SnapSizeMixed, pTmpBuffer4, 1);

					if (m_aDummy[Index].m_ClientID < 0 || m_aDummy[Index].m_ClientID >= MAX_CLIENTS)
						m_aDummy[Index].m_ClientID = -1;

					//dbg_msg(0, "%i", m_aDummy[Index].m_ClientID);
					...
					m_aDummy[Index].m_RecivedSnapshots++;

					m_aDummy[Index].m_CurrentRecvTick = GameTick;

					// we got two snapshots until we see us self as connected
					//if (m_aDummy[Index].m_RecivedSnapshots == 2)
					//{
	...
	//else
		//GameClient()->OnMessageDummy(Msg, &Unpacker);
}
...
void CClient::ProcessServerPacket(CNetChunk *pPacket)
{
	...
	if(Sys)
	{
		...
		if(Msg == NETMSG_MAP_CHANGE)
		{
			...
			else
			{
				...
				else
				{
					...
					m_MapdownloadAmount = 0;
					m_MapdownloadMaxChunk = -1;

					m_MapdownloadQuick = m_MapdlownloadNextType;

					if(m_MapdownloadQuick)
					{
						m_MapdownloadChunks.clear();
						m_NetClient.Connection()->m_AllowTimeout = false;
					}
					else
					{
						CMsgPacker Msg(NETMSG_REQUEST_MAP_DATA);
		...
		else if(Msg == NETMSG_MAP_DATA)
		{
			...
			if(m_MapdownloadQuick == false)
			{
				NewMapChunk(Last, MapCRC, Chunk, Size, pData, true);
			}
			else
			{
				if(MapCRC != m_MapdownloadCrc || Size > 900)
				{//reloadownlad
					m_MapdownloadQuick = false;
					m_MapdownloadAmount = 0;
					m_MapdownloadChunk = 0;
				...
				bool Found = false;
				for(int i = 0; i < m_MapdownloadChunks.size(); i++)
				{
					if(m_MapdownloadChunks[i].m_Chunk == Chunk)
					{
						Found = true;
						break;
					}
				...
				if(!Found && Size >= 0)
				{
					m_MapdownloadChunks.add(CMapDownloadChunk(Chunk, (unsigned char *)pData, Size));
					m_MapdownloadAmount += Size;
				}

				if(Last)
					m_MapdownloadMaxChunk = Chunk;

				//NewMapChunk(Last, MapCRC, Chunk, Size, pData);
			}
		}
		else if(Msg == NETMSG_CON_READY)
		{
			...
			Buffer[sizeof(SERVERBROWSE_GETINFO)] = 0;

			Packet.m_ClientID = -1;
			Packet.m_Address = m_ServerAddress;
			Packet.m_Flags = NETSENDFLAG_CONNLESS;
		...
		else if(Msg == NETMSG_SNAP || Msg == NETMSG_SNAPSINGLE || Msg == NETMSG_SNAPEMPTY)
		{
			...
			if(GameTick >= m_CurrentRecvTick)
			{
				...
				if(m_SnapshotParts == (unsigned)((1<<NumParts)-1))
				{
					...
					unsigned char aTmpBuffer4[CSnapshot::MAX_SIZE];
					CSnapshot *pTmpBuffer3 = (CSnapshot*)aTmpBuffer3;	// Fix compiler warning for strict-aliasing
					CSnapshot *pTmpBuffer4 = (CSnapshot*)aTmpBuffer4;
					int SnapSize;
					int SnapSizeMixed;
					...
					m_SnapshotStorageMixed.PurgeUntil(PurgeTick);
					SnapSizeMixed = SnapSize;
					//mem_copy(aTmpBuffer4, aTmpBuffer3, sizeof(aTmpBuffer4));
					pTmpBuffer4->Copy(pTmpBuffer3);
					...
					if (m_aDummy[0].m_Online && m_aDummy[0].m_RecivedSnapshots >= 3)
					{
						//if (m_aDummy[0].m_CurrentRecvTick > m_CurrentRecvTick)
						{
							CSnapshotStorage::CHolder *pDummySnapshot = m_aDummy[0].m_SnapshotStorage.m_pLast;
							SnapSizeMixed = pTmpBuffer4->Add(pDummySnapshot->m_pSnap);
							//m_SnapshotStorageMixed.m_pLast->m_Tick = m_aDummy[0].m_SnapshotStorage.m_pLast->m_Tick;
							//m_SnapshotStorageMixed.m_pFirst->m_Tick = m_aDummy[0].m_SnapshotStorage.m_pFirst->m_Tick;
						}
					}
...
void CClient::NewMapChunk(int Last, int MapCRC, int Chunk, int Size, const unsigned char *pData, bool RequestNext)
{
	// check fior errors
	if(Size <= 0 || MapCRC != m_MapdownloadCrc || Chunk != m_MapdownloadChunk || !m_MapdownloadFile)
		return;
...
bool CClient::MapdownloadChunkDownloaded(int Chunk)
{
	for(int i = 0; i < m_MapdownloadChunks.size(); i++)
	{
		if(Chunk == m_MapdownloadChunks[i].m_Chunk)
			return true;
	}
	return false;
}

int CClient::MapdownloadFindWantedChunk()
{
	//m_MapdownloadChunksize

	if(m_MapdownloadMaxChunk != -1)
	{
		if(m_MapdownloadChunk > m_MapdownloadMaxChunk)
			m_MapdownloadChunk = 0;

		if(m_MapdownloadChunk != 0)
		{
			bool Found = false;
			for(int i = m_MapdownloadChunk; i < m_MapdownloadMaxChunk; i++)
			{
				if(MapdownloadChunkDownloaded(i) == false)
				{
		...
		if(m_MapdownloadChunk == 0 && m_MapdownloadChunks.size() > 0)
		{
			for(int i = 0; i < m_MapdownloadMaxChunk; i++)
			{//check for finish
				if(MapdownloadChunkDownloaded(i) == false)
				{//found another not downloaded chunk
					m_MapdownloadChunk = i;
					break;
				}
			}

			if(m_MapdownloadChunk == 0)//done?
				return -1;
		}
	}
	else
	{
		int Chunk = m_MapdownloadChunk;
		m_MapdownloadChunk++;
		return Chunk;
	}

	return m_MapdownloadChunk;
}
...
void CClient::PumpNetwork()
{
	...
	static int64 m_QuickDownloadTime = 0;
	if(m_MapdownloadQuick && m_QuickDownloadTime < time_get())
	{
		int Chunk = MapdownloadFindWantedChunk();
		if(Chunk != -1)
		{
			CMsgPacker Msg(NETMSG_REQUEST_MAP_DATA);
			Msg.AddInt(Chunk);
		...
		{//done
			...
			for(int i = 0; i < m_MapdownloadChunks.size(); i++)
			{
				NewMapChunk(i == m_MapdownloadChunks.size()-1, m_MapdownloadCrc, i, m_MapdownloadChunks[i].m_Size, m_MapdownloadChunks[i].m_aData, false);
			}

			m_MapdownloadQuick = false;
			m_NetClient.Connection()->m_AllowTimeout = true;
		}
		
		m_QuickDownloadTime = time_get()+time_freq()*0.001f;
	}
	...
	while(m_NetClient.Recv(&Packet))
	{
		...
		else
		{
			if(m_DummyControl == -1)
				...
				ProcessServerPacketDummy(&Packet, m_DummyControl);
		}
	}
	...
	for(int i = 0; i < MAX_DUMMIES; i++)
	{
		if(m_aDummy[i].m_Online)
		{
			if(m_aDummy[i].m_NetClient.State() == NETSTATE_OFFLINE)
			{
				if(m_aDummy[i].m_LastConnect + time_freq()*1.1f < time_get())
				{
					m_aDummy[i].m_NetClient.Connect(&m_ServerAddress);

					CMsgPacker MsgInfo(NETMSG_INFO);
					MsgInfo.AddString(GameClient()->NetVersion(), 128);
					MsgInfo.AddString(g_Config.m_Password, 128);
					SendMsgDummy(&MsgInfo, MSGFLAG_VITAL|MSGFLAG_FLUSH, i);

					m_aDummy[i].m_LastConnect = time_get();
				}
			}
			else
				m_aDummy[i].m_NetClient.Update();

			mem_zero(&Packet, sizeof(Packet));
			while (m_aDummy[i].m_NetClient.Recv(&Packet) && Packet.m_ClientID != -1)
			{
				if (m_DummyControl == i)
					ProcessServerPacket(&Packet);
				else
					ProcessServerPacketDummy(&Packet, i);
			}
...
void CClient::Update()
{
	if(m_Autojoin)
	{
		if(g_Config.m_PdlAutojoin == 0)
			m_Autojoin = false;

		if(m_AutojoinInfoTime < time_get())
		{
			unsigned char Buffer[sizeof(SERVERBROWSE_GETINFO)+1];
			CNetChunk Packet;
			...
			Packet.m_ClientID = -1;
			Packet.m_Address = m_AutojoinAddress;
			Packet.m_Flags = NETSENDFLAG_CONNLESS;
			Packet.m_DataSize = sizeof(Buffer);
			Packet.m_pData = Buffer;
			...
			m_AutojoinInfoTime = time_get() + time_freq() * 0.05f;
		}
	}
	...
	else if(State() == IClient::STATE_ONLINE && m_RecivedSnapshots >= 3)
	{
		...
		//		CSnapshotStorage::CHolder *pDummySnapshot = m_aDummy[0].m_SnapshotStorage.m_pLast;
		//		CSnapshotStorage::CHolder *pMainSnapshot = m_aSnapshots[SNAP_CURRENT];
		//		pMainSnapshot->m_pAltSnap->Fusion(pDummySnapshot->m_pAltSnap);
		//		/*int Delta = m_SnapshotDelta.UnpackDelta(pDummySnapshot->m_pAltSnap, pMainSnapshot->m_pAltSnap, pDummySnapshot->m_pAltSnap->DataStart(), pDummySnapshot->m_pAltSnap->DataSize());
		//		dbg_msg(0, "Delta %i", Delta);*/
		//	}
		//	/*else
		//		dbg_msg(0, "main");*/
...
bool CClient::IsDDRace()
{
	if(State() != IClient::STATE_ONLINE)
		return false;

	if(str_find_nocase(m_CurrentServerInfo.m_aGameType, "ddrace"))
		return true;
...
void CClient::Run()
{
	...
	*/
	...
	while (1)
	{
		...
		if(Input()->KeyPressed(KEY_LCTRL) && Input()->KeyPressed(KEY_LSHIFT) && Input()->KeyDown('p'))
			ReinitWindow();

		if(Input()->KeyPressed(KEY_LCTRL) && Input()->KeyPressed(KEY_LSHIFT) && Input()->KeyDown('m'))
...
void CClient::Con_PdlDummyToggle(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	int Index = pResult->GetInteger(0);

	pSelf->DummyToggle(Index);
}

void CClient::Con_PdlDummyConnect(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	int Index = pResult->GetInteger(0);

	pSelf->DummyConnect(Index);
}

void CClient::Con_PdlDummyDisconnect(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	int Index = pResult->GetInteger(0);

	pSelf->DummyDisonnect(Index);
}

void CClient::Con_PdlDummyControl(IConsole::IResult *pResult, void *pUserData)
{
	CClient *pSelf = (CClient *)pUserData;
	int Index = pResult->GetInteger(0);
...
void CClient::Con_HideConsole(IConsole::IResult *pResult, void *pUserData)
{
...
void CClient::RegisterCommands()
{
	...
	m_pConsole->Register("pdl_dummy_toggle", "i", CFGFLAG_CLIENT, Con_PdlDummyToggle, this, "Toggle connection of a dummy");
	...
	m_pConsole->Register("hide_console", "i", CFGFLAG_CLIENT, Con_HideConsole, this, "");
...
void CClient::FirstOpen()
{
	...
	if(g_Config.m_SndVolume < 20.0f)
		g_Config.m_SndVolume = 20.0f;
