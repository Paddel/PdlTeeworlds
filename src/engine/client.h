...
public:
	...
	*/
	...
	virtual bool GetAutojoin() = 0;
	virtual NETADDR *GetAutojoinAddr() = 0;
	...
	virtual bool IsDDRace() = 0;

	virtual void DummyToggle(int Index) = 0;
	virtual void DummyConnect(int Index) = 0;
	...
	virtual int SendMsg(CMsgPacker *pMsg, int Flags, bool System = false) = 0;
	virtual int SendMsgDummy(CMsgPacker *pMsg, int Flags, int Index, bool System = true) = 0;

	int SendMsgSpread(CMsgPacker *pMsg, int Flags, bool System = false)
	{
		int Dummy = GetDummyControl();
		if (Dummy == -1)
			return SendMsg(pMsg, Flags, System);
		else
			return SendMsgDummy(pMsg, Flags, Dummy, System);
	}

	template<class T>
	int SendPackMsgSpread(T *pMsg, int Flags)
	{
		CMsgPacker Packer(pMsg->MsgID());
		if (pMsg->Pack(&Packer))
			return -1;

		return SendMsgSpread(&Packer, Flags);
	}
	...
	template<class T>
	int SendPackMsgDummy(T *pMsg, int Flags, int Index)
	{
		CMsgPacker Packer(pMsg->MsgID());
		if (pMsg->Pack(&Packer))
			return -1;
		return SendMsgDummy(&Packer, Flags, Index, false);
	}
...
public:
	...
	virtual CPlayerInfo *GetDummyJoinInfo(int Dummy) = 0;
	virtual void OnDummyOnJoin(int Dummy) = 0;
