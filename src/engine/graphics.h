...
public:
	...
	*/
	...
	virtual void TriangleBegin() = 0;
	virtual void TriangleEnd() = 0;
	virtual void TriangleSetRotation(float Angle) = 0;
	virtual void TriangleDraw(const CTriangleItem *pArray, int Num) = 0;
	...
	void ReloadTextures()
	{
		...
		for(int i = 0; i < m_pTextureUsers.size(); i++)
			m_pTextureUsers[i]->InitTextures();
	}

	void AddTextureUser(CTextureUser *pUser)
	{
		m_pTextureUsers.add( pUser );
	}
};
